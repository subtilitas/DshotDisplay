...
static bool wrCmd(uint16_t reg) {
	BOARD_I2C.beginTransmission(CST328_I2C_ADDR);
	BOARD_I2C.write((uint8_t)(reg >> 8));
	BOARD_I2C.write((uint8_t)(reg & 0xFF));
...
static bool rdRegs(uint16_t reg, uint8_t *buf, uint8_t n) {
	...
	if (BOARD_I2C.requestFrom((uint8_t)CST328_I2C_ADDR, n) != n) return false;
